...
mask_downsample = downsample_img(mask,6)
...
for file_name in file_list:
    ...
    img_downsample = downsample_img(img,6)
    img_blur = gaussian_blur(img_downsample,5.0)
...
np.random.seed(0)
connectivity = grid_to_graph(*dims)
ward = AgglomerativeClustering(n_clusters=5, linkage="ward").fit(df)
label = ward.labels_
