...
def generate_order_list(plot_order):
    
    order = pd.read_csv(path+'subcortical_order.csv').to_numpy()
...
path = '../data/'
overlap_data = pd.read_csv(path+'Fig6_cp_overlap_GPe.csv')
ids = np.unique(overlap_data['Volume1'])
ids = [int(x[:-5]) for x in ids]

metadata = pd.read_csv(path+'CP_anterograde_output.csv')
metadata = metadata[metadata['image series ID'].isin(ids)]
...
metadata.sort_values(by='manually annotated injection site',inplace=True)
...
ticks,order_list = get_ticks(overlap_data,order_list,len(np.unique(vol1_id)))
...
overlap_heatmap(heatmap_table,'../figures/Fig6_cp_overlap_GPe.png',ticks,order_list)
